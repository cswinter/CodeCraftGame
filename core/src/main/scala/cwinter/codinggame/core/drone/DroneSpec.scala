...
case class DroneSpec(
  ...
  private[drone] def constructDynamics(owner: Drone, initialPos: Vector2, time: Double): DroneDynamics =
    new DroneDynamics(owner, maximumSpeed, weight, radius, initialPos, time)

  private[drone] def constructStorage(owner: Drone, startingResources: Int = 0): Option[DroneStorageModule] =
  ...
  private[drone] def constructProcessingModules(owner: Drone): Option[DroneFactoryModule] =
    if (processingModules > 0) {
      val startIndex = storageModules + missileBatteries
      Some(new DroneFactoryModule(startIndex until startIndex + processingModules, owner))
    } else None

  private[drone] def constructManipulatorModules(owner: Drone): Option[DroneManipulatorModule] =
  ...
  private[drone] def constructEngineModules(owner: Drone): Option[DroneEnginesModule] =
    if (engineModules > 0) {
  ...
  private[drone] def constructShieldGenerators(owner: Drone): Option[DroneShieldGeneratorModule] =
