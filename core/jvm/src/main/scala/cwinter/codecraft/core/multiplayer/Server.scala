...
class MultiplayerServer(
  ...
  def receive = {
    ...
    case MatchmakingRequest(client) =>
      val connection =
        connectionInfo.valuesIterator.find(_.worker == client).get
      waitingClient match {
  ...
  private def acceptConnection(rawConnection: ActorRef): Connection = {
    ...
    val websocketActor =
      context.actorOf(WebsocketActor.props(rawConnection, worker))
  ...
  private def gameStatus(sim: DroneWorldSimulator,
      ...
      info.connections.foldLeft(0.0) {
        case (sum, c) =>
          if (closeReason.isEmpty)
            sum + c.worker.outKbps(sim.measuredFramerate / tickPeriod.toDouble)
    ...
    val inBandwidth =
      info.connections.foldLeft(0.0) {
        case (sum, c) =>
          if (closeReason.isEmpty)
            sum + c.worker.inKbps(sim.measuredFramerate / tickPeriod.toDouble)
    ...
    GameStatus(
      ...
      if (info.connections.isEmpty) 0
      else info.connections.map(_.worker.msSinceLastResponse).max,
