...
object ModelFactory {
  def generateModel(worldObject: WorldObject): WorldObjectModel = worldObject match {
    case mineral: MineralObject => new MineralObjectModel(mineral)
  }
}


abstract class WorldObjectModel(worldObject: WorldObject) {
...
class MineralObjectModel(mineral: MineralObject) extends WorldObjectModel(mineral) {
  val size = mineral.size

  val model =
    new Polygon(5, RenderFrame.bloomShader)
